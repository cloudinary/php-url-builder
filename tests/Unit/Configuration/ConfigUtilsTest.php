...
final class ConfigUtilsTest extends UnitTestCase
{
    ...
    {
        $configArr = ConfigUtils::parseCloudinaryUrl($this->cloudinaryUrl . '/' . $this::DOMAIN);

        self::assertCount(3, $configArr['cloud']);

        self::assertCount(1, $configArr['url']);

        self::assertEquals(self::DOMAIN, $configArr['url']['domain']);
    }
